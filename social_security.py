...
# availability: october/late in the second year? (late 2025 for 2024)
...
SnP500AnnualData = {
    ...
    1927: (       0,        0,        0,        0,    17.66,      0),
}

# sanity check annual S&P 500 returns
...
def format_results(results):
    ...
    if res.get('SnP500'):
        ...
        lines.append(f"  Earnings years averaged {snp['AvgEarnYears']}% over {snp['EarnYears']} + {snp['ExtraYears']} years")
...
def snp500():
    ...
    #    earnsum += allavg
    # what to use to last return year, earnmax? should it be to lastsnp?  current?  maybe to FRA? lastsnp means no padding...
    #lastreturnyear = earnmax
    lastreturnyear = lastsnp
    earnsum = sum(PChg for y, (AvgCP, YOpen, YHigh, YLow, YClose, PChg) in SnP500AnnualData.items() if earnmin < y <= lastreturnyear)
    earnavg = earnsum / (earnmax - earnmin + 1 + (lastreturnyear - earnmax))
    ...
    for year, dollar in Results['EarningsRecord'].items():
        ...
        snptotal *= (100 + SnP500AnnualData.get(year, DefaultYear)[SIdxPChg]) / 100
    ...
    Results.update({'SnP500': {
        ...
        "ExtraYears": lastreturnyear - earnmax,
